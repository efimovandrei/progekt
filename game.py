...
def close_screen(vin_ships, k, level):
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                with open('data/results.txt', mode='w', encoding='utf8') as f:
                    if level == 1:
                        print(f'{k} игра; Уровень: Легкий; Получено очков: {vin_ships}', file=f)
                    if level == 2:
                        print(f'{k} игра; Уровень: Средний; Получено очков: {vin_ships}', file=f)
                    if level == 3:
                        print(f'{k} игра; Уровень: Сложныйц; Получено очков: {vin_ships}', file=f)
...
while running:
    ...
    clock.tick(FPS)
    pygame.display.flip()
close_screen(vin_ships, k, level)
terminate()
